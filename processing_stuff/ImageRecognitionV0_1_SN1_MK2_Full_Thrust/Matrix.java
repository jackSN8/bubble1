...
public class Matrix 
{
  ...
  static Matrix dsigmoidfMatrix(Matrix a)
  {
    ...
    for (int i = 0; i < result.rows; i++) 
    {
      ...
      {
        result.values[i][j] = 1;
        //result.values[i][j] = dsigmoidf(a.values[i][j]);
      }
    }
    return result;
  }
  ...
  static float dsigmoidf(float input)
  {
    return input * (1-input);
  }


 static void findDimensions(Matrix a)
  {  
    if(a==null)
    {
    ...
    System.out.println("Matrix colums are ");
