double e = 2.7182818284590452353602874713526624977572470936999595749669676277240766303535475945713829178d;
...
class NeuralNet
{
  ...
  //NN constructor in the read only MNST file takes the actual matrices as inputs
  NeuralNet(int tInputs, int tHiddens, int tOutputs,Matrix TweightsIH, Matrix TweightsHO,Matrix TbiasH,Matrix TbiasO)
  {
    inputNodes = tInputs;
    ...
    weightsIH = TweightsIH;
    weightsHO = TweightsHO;
    
    //Initialization of the biases
    biasH = TbiasH;
    biasO= TbiasO;
    
    //Debug code to check matrix dimensions
    ////Input hidden, should be 20x784
    Matrix.findDimensions(weightsIH);
    ////Hidden output, should be 10x20
    Matrix.findDimensions(weightsHO);
